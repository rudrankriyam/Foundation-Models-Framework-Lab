...
- Run on-device measurements with `FMFBenchCLI` and PCC measurements with the
  signed `FMFBenchDeviceRunner` app.
...
- OS 27 on-device comparison.
- OS 27 PCC service comparison.
- Run every published iPhone measurement with `FMFBenchDeviceRunner` on the
  physical phone.
...
- No omitted failures.
- FMFBench commit SHA recorded alongside the result.
