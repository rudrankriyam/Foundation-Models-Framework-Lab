...
names describe the product pattern that inspired each workload; FMFBench does not
...
DEVELOPER_DIR=/path/to/Xcode-beta.app/Contents/Developer \
  swift run fmfbench --suite quick --model pcc --reasoning moderate \
  --fallback on-device
```
...
and the iOS device runner. A separate macOS 27 package replays recorded FMFBench
...
Official Mac results come from `FMFBenchCLI` through `swift run fmfbench` or the
...
and iPad results therefore use the signed `FMFBenchDeviceRunner` harness on a physical
Apple Intelligence device. Open
`Tools/FMFBench/FMFBenchDeviceRunner/FMFBenchDeviceRunner.xcodeproj`, select the
physical device, and run the `FMFBenchDeviceRunner` scheme.
