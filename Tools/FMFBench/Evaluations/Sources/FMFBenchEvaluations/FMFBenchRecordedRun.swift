...
public struct FMFBenchRecordedRun: Sendable {
  public let info: [String: String]
  public let records: [FMFBenchEvaluationRecord]

  public init(
    info: [String: String],
    records: [FMFBenchEvaluationRecord]
  ) {
    self.info = info
...
public struct FMFBenchEvaluationRecord: Sendable {
...
public enum FMFBenchRecordedRunLoader {
  public static func load(from url: URL) throws -> FMFBenchRecordedRun {
    try decode(
      Data(contentsOf: url),
  ...
  ) throws -> FMFBenchRecordedRun {
    let decoder = JSONDecoder()
    decoder.dateDecodingStrategy = .iso8601

    do {
      let result = try decoder.decode(FMFBenchRunResult.self, from: data)
      ...
      do {
        ...
        throw FMFBenchRecordedRunError.invalidDocument(
  ...
  private static func currentRun(
    _ result: FMFBenchRunResult,
    sourceName: String?
  ) -> FMFBenchRecordedRun {
    let scenarios = Dictionary(
      uniqueKeysWithValues: FMFBenchScenarioCatalog.all.map { ($0.id, $0) }
    )
  ...
  private static func currentRecord(
    _ trial: FMFBenchTrialResult,
    scenarios: [String: FMFBenchScenario]
  ) -> FMFBenchEvaluationRecord {
    let scenario = scenarios[trial.scenarioID]
    return FMFBenchEvaluationRecord(
...
public enum FMFBenchRecordedRunError: LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .invalidDocument(let currentError, let legacyError):
      """
      The file is not a supported FMFBench result.
