...
public enum FMFBenchEvaluationsAdapter {
  ...
  public static let toolCallsPassMetric = Metric("FMFBench Tool Calls Pass")
  public static let toolCallsPercentageMetric = Metric("FMFBench Tool Calls Percentage")

  public static func samples(
    for scenario: FMFBenchScenario
  ) throws -> [FMFBenchEvaluationSample] {
    let schema: GenerationSchema?
    switch scenario.outputMode {
    case .text:
      schema = nil
    case .guided(let fmfBenchSchema):
      schema = try FMFBenchSchemaFactory.make(fmfBenchSchema)
    }

    return try scenario.samples.map { sample in
      FMFBenchEvaluationSample(
        recordID: sample.id,
        prompt: try fmfBenchPrompt(for: sample),
  ...
  public static func toolCallEvaluator(
    for scenario: FMFBenchScenario
  ) -> ToolCallEvaluator<FMFBenchEvaluationSample>? {
  ...
  private static func failedCheckRationale(_ grade: FMFBenchGrade) -> String? {
...
extension ModelSubject where Value == String {
  fileprivate var fmfBenchToolCalls: [FMFBenchToolCall] {
    toolCalls.compactMap { call in
      guard case .structure(let properties, _) = call.arguments.kind else {
        return nil
      }
      let arguments = properties.compactMapValues(\.fmfBenchJSONValue)
      return FMFBenchToolCall(name: call.toolName, arguments: arguments)
    }
  }
}
...
extension GeneratedContent {
  fileprivate var fmfBenchJSONValue: FMFBenchJSONValue? {
    switch kind {
    case .string(let value):
      return .string(value)
    case .number(let value):
