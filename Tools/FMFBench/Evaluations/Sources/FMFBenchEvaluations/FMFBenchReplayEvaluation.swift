...
public struct FMFBenchReplayEvaluation: Evaluation {
  public let executionSuccess = Metric("FMFBench Execution Success")
  public let promptPass = Metric("FMFBench Prompt Pass")
  public let constraintScore = Metric("FMFBench Constraint Score")
  public let safetyPass = Metric("FMFBench Safety Pass")
  public let durationSeconds = Metric("FMFBench Duration Seconds")
  public let timeToFirstTokenSeconds = Metric("FMFBench TTFT Seconds")
  public let outputTokensPerSecond = Metric("FMFBench Output Tokens Per Second")
  public let peakResidentMemoryMiB = Metric("FMFBench Peak Resident Memory MiB")
  public let toolCallsPass = Metric("FMFBench Tool Calls Pass")
  public let toolCallsPercentage = Metric("FMFBench Tool Calls Percentage")
  ...
  public init(run: FMFBenchRecordedRun) throws {
    guard !run.records.isEmpty else {
      throw FMFBenchReplayEvaluationError.emptyRun
    }
    ...
    includesToolExpectations = run.records.contains { record in
      FMFBenchEvaluationsAdapter.trajectoryExpectation(for: record.checks) != nil
    }
    dataset = ArrayLoader(
  ...
  public var evaluators: Evaluators {
    ...
    evaluators.append(
      Evaluator { input, _ in
        ...
        guard
          let passed = FMFBenchSafetyClassifier.passed(
...
extension FMFBenchReplayEvaluation {
  ...
  private func scoringEvaluator(
    metric: Metric,
    value: @escaping @Sendable (FMFBenchEvaluationRecord) -> Double?,
  ...
  private func generatedContent(
    for arguments: [String: FMFBenchJSONValue]
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw FMFBenchReplayEvaluationError.invalidToolArguments
    }
    return try GeneratedContent(json: json)
  }

  private func failedCheckRationale(_ grade: FMFBenchGrade) -> String? {
    let failures = grade.checks.filter { !$0.passed }.map(\.label)
    return failures.isEmpty ? nil : failures.joined(separator: "; ")
  }
}

public enum FMFBenchReplayEvaluationError: LocalizedError {
  case emptyRun
  ...
  public var errorDescription: String? {
    switch self {
    case .emptyRun:
      "The FMFBench result contains no trials or failures."
    case .missingRecord(let id):
      "The evaluation sample does not map to an FMFBench record: \(id ?? "missing ID")."
    case .invalidToolArguments:
