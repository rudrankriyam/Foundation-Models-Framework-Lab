...
struct AppBenchEvaluateCLI {
  static func main() async {
    do {
      ...
      fputs("appbench-evaluate: \(error.localizedDescription)\n", stderr)
      exit(1)
    }
  }
  ...
  private static func replay(_ arguments: [String]) async throws {
    ...
    let input = try parser.requiredPath(label: "AppBench JSON result")
    try parser.finish()

    let output =
      outputOption.map(expandedURL)
      ?? input.deletingLastPathComponent().appending(path: "Evaluations")
    let run = try AppBenchRecordedRunLoader.load(from: input)
    ...
    switch format {
    case .text:
      print("Replayed \(run.records.count) recorded AppBench sample(s).")
  ...
  private static func printUsage() {
    print(
      """
      Usage:
        appbench-evaluate replay <appbench.json> [--output <directory>]
            [--no-report-metadata] [--format text|json]

      Replays recorded AppBench responses through Apple Evaluations without
...
private struct ReplayPayload: Encodable {
  let schemaVersion = "appbench-evaluate/v1"
  let command = "replay"
  let source: String
  let sampleCount: Int
...
private struct ArgumentParser {
  ...
  mutating func requiredPath(label: String) throws -> URL {
    ...
    return AppBenchEvaluateCLI.expandedURL(arguments[index])
  }
