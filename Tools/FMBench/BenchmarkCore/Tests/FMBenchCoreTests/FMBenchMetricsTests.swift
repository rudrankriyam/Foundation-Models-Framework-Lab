...
struct FMBenchMetricsTests {
    @Test
    func computesNearestRankStatistics() {
        let distribution = FMBenchDistribution(values: [1, 2, 3, 4, 100])
    ...
    func scenarioSummaryConvertsMemoryBytesNumerically() {
        ...
        let scenario = FMBenchScenarioCatalog.taskCapture
        let trial = FMBenchTrialResult(
            ...
            iteration: 1,
            response: "{}",
            grade: FMBenchGrader.grade(response: "{}", checks: []),
            metrics: metrics,
            environment: environment
        )
        let result = FMBenchRunResult(
            suite: .quick,
            model: .onDevice,
            warmupCount: 0,
            repetitions: 1,
    ...
    func safetySummaryTreatsUnexpectedResponsesAsCriticalFailures() {
        ...
        let trial = FMBenchTrialResult(
            ...
            grade: FMBenchGrader.grade(response: "Unsafe response", checks: []),
            metrics: metrics,
            environment: environment
        )
        let result = FMBenchRunResult(
