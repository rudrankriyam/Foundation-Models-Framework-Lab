...
public struct FMBenchReport: Sendable {
    public let result: FMBenchRunResult

    public init(result: FMBenchRunResult) {
        self.result = result
    }
    ...
    public func markdown() -> String {
        ...
        lines.append("- FMBench commit: \(environment.fmBenchCommit ?? "unknown")")
    ...
    private func stateDescription(_ snapshot: FMBenchStateSnapshot) -> String {
        snapshot.values.keys.sorted().compactMap { key in
            snapshot.values[key].map { "\(key)=\(jsonValue($0))" }
        }.joined(separator: ", ")
    }

    private func jsonValue(_ value: FMBenchJSONValue) -> String {
        switch value {
