...
Foundation Models FMBench measures real application workloads across Apple devices,
OS releases, the on-device system model, and Private Cloud Compute.

FMBench is maintained in
[`Tools/FMBench`](https://github.com/rudrankriyam/Foundation-Models-Framework-Lab/tree/main/Tools/FMBench)
...
Guided generation structure is not counted as quality. FMBench grades the semantic
...
systems and PCC runs, FMBench records a calibrated character estimate and marks
...
swift run fmbench --suite quick --all-samples --model on-device

# Full 250-sample practical corpus with export
swift run fmbench --suite full --warmups 5 --repetitions 20 \
  --json Tools/FMBench/Results/macbook-m5-macos-27.json \
  --markdown Tools/FMBench/Results/macbook-m5-macos-27.md
...
swift run fmbench --suite context --connectivity offline
...
Set `FMBENCH_DEVICE_NAME` when you want a friendly public label; otherwise
FMBench uses the non-personal hardware identifier rather than the machine
hostname.

To pair a run with Apple's Foundation Models Instrument:

```bash
Tools/FMBench/BenchmarkCore/run-trace.sh \
...
```bash
# Create a native evaluation result from a portable FMBench JSON report.
Tools/FMBench/fmbench-evaluate replay \
  Tools/FMBench/Results/run.json \
  --output /tmp/fmbench-evaluations \
  --format json
...
# FMBENCH_RESULT is relative to Tools/FMBench.
xceval pipeline Tools/FMBench/xceval.pipeline.json \
  --set FMBENCH_RESULT=Results/run.json \
  --force
```

The generic [`xceval`](https://github.com/rudrankriyam/Evaluations-Framework-CLI)
CLI is a separate public tool and does not know about FMBench’s JSON schema.
See
[FMBench and Apple Evaluations](docs/EVALUATIONS.md) for the framework locations,
...
without an OS update, so every result retains its timestamp and OS build. FMBench
...
this single-device baseline. Pre-FMBench community measurements are preserved
...
- `FMBenchCore`: scenarios, graders, runner, statistics, and reports.
- `BenchmarkCore`: compatibility product that exposes the `FMBenchCore` module.
- `fmbench`: command-line experiment runner backed by the `FMBenchCLI` target.

The nested `BenchmarkCore/Package.swift` exports the same portable products and keeps
the original `FMBenchCLI` executable product for focused package development.
`Tools/FMBench/Evaluations/Package.swift` is a separate macOS 27 developer-tool
package that exports `FMBenchEvaluations` and the FMBench-specific
`fmbench-evaluate` replay command. Generic artifact tooling lives in `xceval`.
