# FMBench and Apple Evaluations

FMBench uses Apple’s Evaluations framework as a **macOS 27 evaluation control
...
1. `FMBenchCore` runs deployment-shaped workloads and records portable JSON.
2. The macOS-only `FMBenchEvaluations` package replays those recorded responses
   through deterministic Evaluations metrics.
3. The FMBench-specific `fmbench-evaluate` command creates native
   `.xcevalresult` files.
...
original FMBench trial.

Apple makes Evaluations available on macOS, iOS, watchOS, and visionOS 27, with
tvOS unavailable. FMBench’s macOS-only boundary is an intentional product and
tooling decision, not a claim that Apple lacks other platform binaries. The
shipping signed runner continues to use only `FMBenchCore` on iOS and macOS.
...
FMBench runner
  |
  | portable FMBench JSON
  v
fmbench-evaluate replay (macOS 27)
  |
  | Evaluation.run(info:)
  v
.xcevalresult JSON
...
The FMBench package manifest adds the developer framework search path and an Xcode
`Contents` runpath. `Tools/FMBench/fmbench-evaluate` discovers Xcode installations
...
FMBench keeps its internal record ID in the serialized input sample and leaves
the framework's `Expected` column empty. FMBench workloads usually express
...
FMBench currently uses deterministic evaluators and native tool trajectory
...
FMBench owns only the producer command because it understands FMBench’s portable
result schema and deterministic graders:

```bash
# Convert a recorded FMBench run into a native evaluation result.
...
xceval inspect \
  /tmp/fmbench-evaluations/FMBenchReplayEvaluation-*.xcevalresult \
  --output json
...
The checked-in pipeline runs the FMBench replay command and writes one
...
`fmbench-evaluate replay` accepts both the current FMBench result schema and the
repository’s historical June 2026 result schema. `xceval compare` reports raw
deltas without assuming whether higher or lower is better; an FMBench policy
layer can apply metric-specific regression thresholds later.

Warmup failures remain in the portable FMBench report for diagnostics but are
...
- Execution success.
- Deterministic FMBench prompt pass.
- Deterministic FMBench constraint score.
...
- Ordered agentic trajectories, with mocked final-state checks replayed by FMBench's
