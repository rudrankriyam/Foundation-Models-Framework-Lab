...
FMBench's Xcode 27-built PCC request failed before first output with:
...
- Record network type and approximate region manually until FMBench captures
  them.
...
- When using `--connectivity offline`, disable connectivity outside FMBench.
  FMBench verifies that no active network path is available before the run, but it
