...
other user data. FMBench resets the world before every measured trial and preserves its
...
`TrajectoryExpectation` values while the custom evaluator retains FMBench's recorded
final-state checks.
...
- **Mac on-device:** run `FMBenchCLI` through `swift run fmbench` or
  `./Tools/FMBench/fmbench`.
- **Mac PCC:** run the signed `FMBenchDeviceRunner` app on the physical Mac. A
