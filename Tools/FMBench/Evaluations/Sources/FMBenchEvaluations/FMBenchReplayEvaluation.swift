...
public struct FMBenchReplayEvaluation: Evaluation {
  public let executionSuccess = Metric("FMBench Execution Success")
  public let promptPass = Metric("FMBench Prompt Pass")
  public let constraintScore = Metric("FMBench Constraint Score")
  public let safetyPass = Metric("FMBench Safety Pass")
  public let durationSeconds = Metric("FMBench Duration Seconds")
  public let timeToFirstTokenSeconds = Metric("FMBench TTFT Seconds")
  public let outputTokensPerSecond = Metric("FMBench Output Tokens Per Second")
  public let peakResidentMemoryMiB = Metric("FMBench Peak Resident Memory MiB")
  public let toolCallsPass = Metric("FMBench Tool Calls Pass")
  public let toolCallsPercentage = Metric("FMBench Tool Calls Percentage")

  public let dataset: ArrayLoader<FMBenchEvaluationSample>
  public let run: FMBenchRecordedRun
  private let recordsByID: [String: FMBenchEvaluationRecord]
  ...
  public init(run: FMBenchRecordedRun) throws {
    guard !run.records.isEmpty else {
      throw FMBenchReplayEvaluationError.emptyRun
    }
    ...
    includesToolExpectations = run.records.contains { record in
      FMBenchEvaluationsAdapter.trajectoryExpectation(for: record.checks) != nil
    }
    dataset = ArrayLoader(
      samples: run.records.map { record in
        FMBenchEvaluationSample(
          ...
          expectations: FMBenchEvaluationsAdapter.trajectoryExpectation(
            for: record.checks
          )
        )
      }
  ...
  public func subject(
    from sample: FMBenchEvaluationSample
  ...
  public var evaluators: Evaluators {
    ...
    evaluators.append(
      Evaluator { input, _ in
        ...
        guard
          let passed = FMBenchSafetyClassifier.passed(
...
@available(macOS 27.0, *)
extension FMBenchReplayEvaluation {
  private func record(
    for sample: FMBenchEvaluationSample
  ) throws -> FMBenchEvaluationRecord {
    guard let record = recordsByID[sample.recordID] else {
      throw FMBenchReplayEvaluationError.missingRecord(sample.recordID)
    }
    return record
  }
  ...
  private func scoringEvaluator(
    metric: Metric,
    value: @escaping @Sendable (FMBenchEvaluationRecord) -> Double?,
  ...
  private func generatedContent(
    for arguments: [String: FMBenchJSONValue]
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw FMBenchReplayEvaluationError.invalidToolArguments
    }
    return try GeneratedContent(json: json)
  }

  private func failedCheckRationale(_ grade: FMBenchGrade) -> String? {
    let failures = grade.checks.filter { !$0.passed }.map(\.label)
    return failures.isEmpty ? nil : failures.joined(separator: "; ")
  }
}

public enum FMBenchReplayEvaluationError: LocalizedError {
  case emptyRun
  ...
  public var errorDescription: String? {
    switch self {
    case .emptyRun:
      "The FMBench result contains no trials or failures."
    case .missingRecord(let id):
      "The evaluation sample does not map to an FMBench record: \(id ?? "missing ID")."
    case .invalidToolArguments:
