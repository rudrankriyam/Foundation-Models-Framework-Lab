...
guard let developerDirectory else {
  fatalError(
    """
    Xcode 27 with Evaluations.framework was not found. Run ../fmbench-evaluate,
...
let package = Package(
  ...
  products: [
    .library(
      name: "FMBenchEvaluations",
      targets: ["FMBenchEvaluations"]
    ),
    .executable(
      name: "fmbench-evaluate",
      targets: ["FMBenchEvaluateCLI"]
    )
  ],
  dependencies: [
  ...
  targets: [
    .target(
      name: "FMBenchEvaluations",
      dependencies: [
        .product(name: "FMBenchCore", package: "BenchmarkCore")
      ],
      swiftSettings: evaluationSwiftSettings,
      linkerSettings: evaluationLinkerSettings
    ),
    .executableTarget(
      name: "FMBenchEvaluateCLI",
      dependencies: ["FMBenchEvaluations"],
      swiftSettings: evaluationSwiftSettings
    ),
    .testTarget(
      name: "FMBenchEvaluationsTests",
      dependencies: [
        "FMBenchEvaluations",
        .product(name: "FMBenchCore", package: "BenchmarkCore")
      ],
