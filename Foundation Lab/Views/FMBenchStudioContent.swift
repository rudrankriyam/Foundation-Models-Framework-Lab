...
struct FMBenchStudioContent: View {
    ...
    private var settingsContent: some View {
        VStack(alignment: .leading, spacing: Spacing.xLarge) {
            section(title: "Execution Surfaces") {
                VStack(spacing: 0) {
                    detailRow(title: "Mac", value: "fmbench CLI")
                    Divider()
                    detailRow(title: "Private Cloud Compute", value: "Signed FMBenchDeviceRunner")
                    Divider()
                    detailRow(title: "iPhone and iPad", value: "Signed FMBenchDeviceRunner")
                    Divider()
    ...
    private var runsContent: some View {
        VStack(alignment: .leading, spacing: Spacing.xLarge) {
            section(title: "Mac CLI") {
                command("swift run fmbench --suite quick --model on-device")
            }

            section(title: "Physical Device") {
                VStack(alignment: .leading, spacing: Spacing.medium) {
                    Text("Open the FMBenchDeviceRunner project, select a physical Apple Intelligence device, and run its scheme.")
                    .font(.callout)
                    .foregroundStyle(.secondary)

                    command("Tools/FMBench/FMBenchDeviceRunner/FMBenchDeviceRunner.xcodeproj")
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: Spacing.xLarge) {
            section(title: "Artifacts") {
                VStack(spacing: 0) {
                    ...
                    Divider()
                    detailRow(title: "Curated results", value: "Tools/FMBench/Results")
                }
            }

            command(
                "swift run fmbench --suite full "
...
#Preview {
    ScrollView {
        FMBenchStudioContent(stage: .settings)
            .padding()
    }
}
