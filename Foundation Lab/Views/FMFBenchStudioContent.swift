    ...
    private var runsContent: some View {
        VStack(alignment: .leading, spacing: Spacing.xLarge) {
            section(title: "Mac CLI") {
                command("swift run fmfbench --suite quick --model on-device")
            }

            section(title: "Physical Device") {
                VStack(alignment: .leading, spacing: Spacing.medium) {
                    Text("Open the FMFBenchDeviceRunner project, select a physical Apple Intelligence device, and run its scheme.")
                    .font(.callout)
                    .foregroundStyle(.secondary)

                    command("Tools/FMFBench/FMFBenchDeviceRunner/FMFBenchDeviceRunner.xcodeproj")
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: Spacing.xLarge) {
            section(title: "Artifacts") {
                VStack(spacing: 0) {
                    ...
                    Divider()
                    detailRow(title: "Curated results", value: "Tools/FMFBench/Results")
                }
            }

            command(
                "swift run fmfbench --suite full "
...
#Preview {
    ScrollView {
        FMFBenchStudioContent(stage: .settings)
            .padding()
    }
}
